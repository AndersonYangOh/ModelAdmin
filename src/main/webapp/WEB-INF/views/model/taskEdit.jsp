...
function getTaskInfo() {
	...
	$.ajax({
        ...
        success :function (data) {
            ...
            $("#parentId").val(list.taskInfo.parentId);
            $(':radio[name="isReceiveFlag"][value='+list.taskInfo.isReceiveFlag+']').attr('checked', 'true');
            $(':radio[name="inuse"][value='+list.taskInfo.inuse+']').attr('checked', 'true');
            $("#taskId").val(list.taskInfo.taskId).attr("checked",'checked');
            ...
            if (list.taskInfo.isReceiveFlag == 1){
            	$("#lc1").removeClass("ystep-step-undone").addClass("ystep-step-done");
            	document.getElementById("lc2").style.width="66%";
            }
...
function save(type) {
    ...
    var isReceiveFlag=$('input[name="isReceiveFlag"]:checked').val();
    var taskScene=$("#taskScene").val();
    var result=$("#result").val();
    var type = "updateChild";
    if (null == taskScene || taskScene == ''){
    	alert("应用场景不能为空！");
    	return false;
    }
    ...
    $.ajax({
        ...
        success : function (data) {
            if(data.flag==true){
            	$.jBox.tip("修改子作业成功","success");
                setTimeout(function(){window.location.href='model/toTaskDetailList.do?type=1&id='+parentId},3000);
            }else {
                $.jBox.error("修改子作业失败", '提示');
                return false;
            }
        },
        error : function () {
	...
	<div class="u-body">
		...
		<div class="data d2">
			<form>
				<div class="stepCont stepCont2" style="overflow: hidden;">
					...
					<div class="ystep-container ystep-lg ystep-blue">
						<ul class="ystep-container-steps" >
							<li class="ystep-step-done">创建作业</li>
							<li class="ystep-step-done">总部已下发</li>
							<li class="ystep-step-undone" id="lc1">省分是否收到</li>
							<li class="ystep-step-undone" id="lc">省分反馈</li>
						</ul>
						<div class="ystep-progress">
							<p class="ystep-progress-bar">
								<span class="ystep-progress-highlight" id="lc2" style="width: 33%;"></span>
							</p>
						</div>
					</div>
				</div>
				...
				<div class="biaodan">
					<label class="name n1" for="">作业ID：</label>
					<input type="text" class="b-input-text" name="taskId" id="taskId" readonly="readonly"></input>
					<input type="hidden" class="b-input-text" id="parentId" name="parentId" /> 
					<input type="hidden" class="b-input-text" id="id1" name="id1" value="${id}" /> 
				</div>
				<div class="biaodan">
					<label class="name n1" for="">下发省份：</label>
					<input type="text" class="b-input-text" name="provinceName" id="provinceName" readonly="readonly"></input>
					<input type="hidden" class="b-input-text" id="province" name="province" value="${province}"/>
				</div>
				<div class="biaodan">
					<label class="name n1" for="">下发月份：</label>
					<input type="text" class="b-input-text" name="sendMonth" id="sendMonth" readonly="readonly"></input>
				</div>
				<div class="biaodan">
					<label class="name n1" for="">下发数量：</label>
					<input type="text" class="b-input-text" name="sendNum" id="sendNum" readonly="readonly"></input>
				</div>
				  
				 <div class="biaodan">
					<label class="name n1" for="">是否接收：</label>
					<div class="box">
					 	<label class="radio-inline">
							<input type="radio"  value="1" name="isReceiveFlag"/>&nbsp;&nbsp;是&nbsp;&nbsp;
					  	</label>
					    <label class="radio-inline">
							<input type="radio" value="0" name="isReceiveFlag" />&nbsp;&nbsp;否&nbsp;&nbsp;
						</label>
					</div>
				</div> 
				  
				<div class="biaodan">
					<label class="name n1" for="">是否应用：</label>
					<div class="box">
					 	<label class="radio-inline">
							<input type="radio"  value="1" name="inuse" />&nbsp;&nbsp;是&nbsp;&nbsp;
					  	</label>
					    <label class="radio-inline">
