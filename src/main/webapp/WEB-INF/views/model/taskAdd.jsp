...
function FormatDate (strTime) {
	...
	$(document).ready(function() {
		...
		$.ajax({  
	        type: "POST",  
	        url: "model/getModelName.do",   
	        data: params,
			datatype: "json",
	        success: function(data){ 
	        	if (data!=null){
	        		var sourceList = data.rows;
	        		for(x in sourceList){
	        			$("#name").append("<option value="+sourceList[x].id+">" + sourceList[x].name + "</option>");  
	        		}     
	        	}
	        	getModelId();
	        }  
		...
		<div class="data d2">
			<form>
				<div class="biaodan">
					...
					<select id="name" name="name" class="b-input-text" onChange="getModelId();">
	                </select>
				</div>
				<div class="biaodan">
					<label class="name">模型id：</label>
					<input  class="b-input-text" type="text" value="0000" name="modelId" readonly id="modelId" style="width:392px"/>
				</div>
				<div class="biaodan">
					<label class="name">下发月份：</label> 
					<select id="sendMonth" name="sendMonth" class="b-input-text" style="width:150px">
